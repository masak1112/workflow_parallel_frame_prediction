...
def directory_scanner(source_path):
    ...
    for dirpath, dirnames, filenames in os.walk(source_path):
        sub_dir_list.extend(dirnames)
        sub_dir_list.sort()
        print ("directory names in source path", dirnames)
        for d in dirnames:
            try:
                path = source_path + d
                num_files = 0
    ...
    total_num_directories = int(len(sub_dir_list))
...
def grib_2_netcdf(rot_grid, source_path, destination_dir, job_name):
    # job_name is the name of the subdirectory that is going to be processed 
    print(' ============= GRIB to netcdf is starting here =============== ')
    ...
    for file_to_process in filenames:
        ...
        temp = file_to_process.split(".")
        base_file_name = temp[0] # TODO with removed suffix from the file name 
        print(base_file_name)
        grb2nc_msg = ("cdo -s -O -f nc4 setgrid  " + file_to_process + " " + rot_grid + " $input_file $new_input_file")
        print(grb2nc_msg)


        # if file extension is bla , bla 

            
        #print(grb2nc_msg)

    #grb2nc_msg = ("cdo -s -O -f nc4 setgrid " + source_path + job_name + "/" + " " + destination_dir + "/" + job_name)
    #if grb2nc_status == 1:
    #    os.system(grb2nc_msg)
